...
class Control:
    ...
    def __init__(self, store: PicoStore, display: Display) -> None:
        ...
        store.subscribe(ACTIONS.SET_STATE.format(
            "onboard_led"), self.on_onboard_change)
        store.subscribe(ACTIONS.SET_STATE.format(
            "pulse_led"), self.on_pulse_led_change)
        store.subscribe(ACTIONS.PULSE_STOP, self.on_pulse_stop)

    def on_onboard_change(self) -> None:
        turn_on = self._store.get_state("onboard_led")
        if turn_on:
            self._onboard.value(1)
            self._display.display_message("onboard led on")
        else:
            self._onboard.value(0)
            self._display.display_message("onboard led off")

    def on_pulse_led_change(self) -> None:
        self._pulse.stop()
        rate = self._store.get_state("pulse_led")
        if rate != None:
            rate = int(rate)
